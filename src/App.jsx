...
const App = () => {
  ...
  const [checkoutForm, setCheckoutForm] = useState({ name: '', phone: '', type: 'Takeaway' });
  ...
  const updateQty = (itemName, change) => {
    setCart(prev => prev.map(i => i.name === itemName ? { ...i, qty: Math.max(1, i.qty + change) } : i));
  };
  ...
  const handleWhatsAppCheckout = (e) => {
    ...
    window.open(`https://wa.me/919580463211?text=${encodeURIComponent(message)}`, '_blank');
    setCart([]);
    ...
    <div className={`min-h-screen font-sans transition-colors duration-500 ${theme.bg} ${theme.text}`}>
      ...
      {isCheckoutOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
           ...
           <div className={`relative w-full max-w-lg ${theme.cardBg} border ${theme.border} p-8 rounded-lg shadow-2xl animate-in zoom-in duration-300`}>
              <h3 className={`text-2xl font-serif mb-6 text-center ${theme.text}`}>Confirm Your Order</h3>
              <form onSubmit={handleWhatsAppCheckout} className="space-y-6">
                ...
                <div><label className={`text-xs uppercase tracking-widest ${theme.subText} mb-2 block`}>Order Type</label><select value={checkoutForm.type} onChange={e => setCheckoutForm({...checkoutForm, type: e.target.value})} className={`w-full bg-transparent border-b ${theme.border} py-3 focus:border-[#C5A059] outline-none ${theme.text}`}><option className="text-black">Takeaway (Pickup)</option><option className="text-black">Dine-In (At Restaurant)</option></select></div>
                ...
                <button type="button" onClick={() => setIsCheckoutOpen(false)} className={`w-full py-2 ${theme.subText} text-xs hover:text-red-500`}>Cancel</button>
              </form>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-sm ${theme.cardBg} border ${theme.border} p-8 rounded-lg shadow-2xl animate-in zoom-in`}>
             <div className="text-center mb-6">
               <Lock size={40} className="mx-auto text-[#C5A059] mb-4" />
               <h3 className={`text-xl font-serif ${theme.text}`}>Admin Access</h3>
             </div>
             <form onSubmit={handleAdminLogin}>
               <input 
                 ...
                 className={`w-full bg-transparent border-b ${theme.border} py-3 text-center text-xl tracking-widest outline-none ${theme.text} mb-8`} 
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 text-sm">
          <div className="col-span-1 md:col-span-1">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-2xl font-serif font-bold tracking-widest text-[#C5A059]">RAJWADA</h3>
            </div>
            <p className="text-gray-400 leading-relaxed">
              Experience the royal taste of tradition. Located in the heart of Jagdishpur.
            </p>
          </div>
          <div>
            <h4 className="tracking-widest uppercase mb-6 font-bold text-[#C5A059]">Contact</h4>
            <ul className="text-gray-400 space-y-3">
              <li>Near Maruti Showroom, Village- Kathaura</li>
              <li>BHEL Jagdishpur, Distt.- Amethi</li>
              <li>+91 95804 63211</li>
            </ul>
          </div>
          <div>
             <h4 className="tracking-widest uppercase mb-6 font-bold text-[#C5A059]">Quick Links</h4>
             <ul className="text-gray-400 space-y-3">
               <li className="hover:text-[#C5A059] cursor-pointer" onClick={() => setActiveTab('Menu')}>Royal Menu</li>
               <li className="hover:text-[#C5A059] cursor-pointer" onClick={() => setActiveTab('Reviews')}>Reviews</li>
             </ul>
          </div>
          <div>
            <h4 className="tracking-widest uppercase mb-6 font-bold text-[#C5A059]">Hours</h4>
            <p className="text-gray-400">Open Daily: 10:00 AM - 10:00 PM</p>
          </div>
        </div>
...
const AdminPanel = ({ theme, menuData, setMenuData, sweetsData, setSweetsData, onLogout }) => {
  ...
  const handleAddItem = (e) => {
    ...
    if (adminTab === 'menu') {
      ...
      if (!updatedMenu[newItem.category]) updatedMenu[newItem.category] = [];
      updatedMenu[newItem.category].push({ ...newItem });
      setMenuData(updatedMenu);
    } else {
      setSweetsData([...sweetsData, { ...newItem }]);
    }
    setNewItem({ category: 'Specials', name: '', price: '', desc: '', img: '' });
  ...
  return (
    <div className={`pt-32 pb-20 px-6 max-w-7xl mx-auto min-h-screen ${theme.text}`}>
      <div className="flex justify-between items-center mb-12 border-b border-gray-700 pb-6">
        <div>
          <h2 className="text-3xl font-serif text-[#C5A059] mb-2">Admin Dashboard</h2>
          <p className={`${theme.subText} text-sm`}>Manage your restaurant content live.</p>
        </div>
        <button onClick={onLogout} className="flex items-center gap-2 bg-red-600/20 text-red-500 px-4 py-2 rounded-lg hover:bg-red-600 hover:text-white transition-all">
          <LogOut size={18} /> Logout
        </button>
      </div>

      <div className="flex gap-4 mb-8">
        <button onClick={() => setAdminTab('menu')} className={`px-6 py-2 rounded-lg font-bold ${adminTab === 'menu' ? 'bg-[#C5A059] text-white' : 'bg-gray-800 text-gray-400'}`}>Food Menu</button>
        <button onClick={() => setAdminTab('sweets')} className={`px-6 py-2 rounded-lg font-bold ${adminTab === 'sweets' ? 'bg-[#C5A059] text-white' : 'bg-gray-800 text-gray-400'}`}>Sweets</button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        {/* ADD ITEM FORM */}
        <div className={`${theme.cardBg} p-8 border ${theme.border} rounded-lg h-fit`}>
          <h3 className="text-xl font-bold mb-6 flex items-center gap-2"><Plus size={20} className="text-[#C5A059]"/> Add New Item</h3>
          <form onSubmit={handleAddItem} className="space-y-4">
             {adminTab === 'menu' && (
               <div>
                 <label className="text-xs text-gray-500 block mb-1">Category</label>
                 <select value={newItem.category} onChange={e => setNewItem({...newItem, category: e.target.value})} className={`w-full p-2 rounded bg-transparent border ${theme.border} outline-none text-sm`}>
                   <option className="text-black">Chaat</option>
                   <option className="text-black">Specials</option>
                 </select>
               </div>
             )}
        ...
        <div className="lg:col-span-2 space-y-8">
          {adminTab === 'menu' ? Object.keys(menuData).map(cat => (
            <div key={cat}>
              <h3 className="text-[#C5A059] font-bold tracking-widest uppercase mb-4 text-sm border-b border-gray-800 pb-2">{cat}</h3>
              <div className="space-y-4">
                {menuData[cat].map((item, idx) => (
                  <div key={idx} className={`flex justify-between items-center p-4 ${theme.cardBg} border ${theme.border} rounded-lg group`}>
                     <div>
                       <h4 className="font-bold">{item.name}</h4>
                       <p className="text-sm text-[#C5A059]">{item.price}</p>
                     </div>
                     <button onClick={() => handleDeleteItem(cat, idx)} className="text-gray-600 hover:text-red-500 p-2"><Trash2 size={18} /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
               {sweetsData.map((item, idx) => (
                 <div key={idx} className={`p-4 ${theme.cardBg} border ${theme.border} rounded-lg relative group`}>
                    <h4 className="font-bold">{item.name}</h4>
                    <p className="text-sm text-[#C5A059]">{item.price}</p>
                    <button onClick={() => handleDeleteSweet(idx)} className="absolute top-2 right-2 text-gray-600 hover:text-red-500"><Trash2 size={16} /></button>
                 </div>
               ))}
            </div>
          )}
...
// --- EXISTING SECTIONS (Props Updated) ---
const HomeSection = ({ setActiveTab, theme }) => (
  <section className="relative">
    <div className="h-screen w-full relative overflow-hidden flex items-center justify-center">
      <div className="absolute inset-0 bg-cover bg-center transition-transform duration-[10000ms] hover:scale-110" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1585937421612-70a008356f36?q=80&w=2000&auto=format&fit=crop')` }} />
      <div className={`absolute inset-0 ${theme.bg === 'bg-[#0F0F0F]' ? 'bg-black/70' : 'bg-black/40'}`} />
      <div className="relative z-10 text-center px-6 max-w-4xl animate-in fade-in slide-in-from-bottom-10 duration-1000">
        <Crown size={64} className="mx-auto mb-8 text-[#C5A059]" />
        <h4 className="text-[#C5A059] tracking-[0.4em] uppercase text-sm mb-6 font-medium">Authentic Indian Cuisine</h4>
        <h1 className="text-white text-5xl md:text-8xl font-serif mb-8 leading-tight">Royal Taste of <br/> Jagdishpur</h1>
        <div className="flex flex-col md:flex-row gap-6 justify-center">
          <button onClick={() => setActiveTab('Menu')} className="px-10 py-4 bg-[#C5A059] text-white tracking-[0.2em] uppercase text-xs font-bold hover:bg-white hover:text-black transition-all">Order Now</button>
          <button onClick={() => setActiveTab('Events')} className="px-10 py-4 border border-white text-white tracking-[0.2em] uppercase text-xs font-bold hover:bg-white hover:text-black transition-all">Book Table</button>
        </div>
      </div>
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-white animate-bounce"><ChevronDown size={32} /></div>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-3 -mt-20 relative z-20 max-w-6xl mx-auto px-6 gap-0 shadow-2xl">
      <div className={`${theme.cardBg} p-12 text-center border-r ${theme.border}`}><ChefHat className="mx-auto mb-6 text-[#C5A059]" size={40} /><h3 className={`text-xl font-serif mb-4 tracking-widest ${theme.text}`}>Rajwada Specials</h3><p className={`${theme.subText} text-sm`}>Try our Soya Chaap & Paneer Angara.</p></div>
      <div className={`${theme.cardBg} p-12 text-center border-r ${theme.border}`}><Utensils className="mx-auto mb-6 text-[#C5A059]" size={40} /><h3 className={`text-xl font-serif mb-4 tracking-widest ${theme.text}`}>Famous Chaat</h3><p className={`${theme.subText} text-sm`}>Aloo Tikki, Pani Batasha & more.</p></div>
      <div className={`${theme.cardBg} p-12 text-center`}><Cake className="mx-auto mb-6 text-[#C5A059]" size={40} /><h3 className={`text-xl font-serif mb-4 tracking-widest ${theme.text}`}>Mithai & Lassi</h3><p className={`${theme.subText} text-sm`}>Traditional sweets & Dry Fruit Lassi.</p></div>
    </div>
  </section>
);

const MenuSection = ({ theme, addToCart, menuData }) => {
  const [filter, setFilter] = useState('Chaat');
  return (
    <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto min-h-screen animate-in fade-in duration-700">
      <div className="text-center mb-16">
        <h4 className="text-[#C5A059] tracking-[0.3em] uppercase text-xs mb-4">The Royal Kitchen</h4>
        <h2 className={`text-4xl md:text-5xl font-serif mb-8 ${theme.text}`}>Our Menu</h2>
        <div className="flex flex-wrap justify-center gap-4">
          {Object.keys(menuData).map(cat => (
            <button key={cat} onClick={() => setFilter(cat)} className={`px-8 py-3 rounded-sm text-xs tracking-widest uppercase transition-all border font-bold ${filter === cat ? 'bg-[#C5A059] text-white border-[#C5A059]' : `${theme.border} ${theme.text} hover:border-[#C5A059]`}`}>{cat}</button>
          ))}
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
        {(menuData[filter] || []).map((item, idx) => (
          <div key={idx} className={`group flex gap-4 ${theme.cardBg} p-4 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border ${theme.border}`}>
            <div className="w-24 h-24 md:w-32 md:h-32 shrink-0 overflow-hidden rounded-md relative bg-gray-800">
               {item.img ? <img src={item.img} alt={item.name} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500" /> : <div className="w-full h-full flex items-center justify-center text-gray-600"><ImageIcon /></div>}
            </div>
            <div className="flex flex-col justify-between flex-grow">
              <div>
                <div className="flex justify-between items-start mb-2"><h3 className={`text-lg font-serif tracking-wide group-hover:text-[#C5A059] transition-colors ${theme.text}`}>{item.name}</h3><span className="text-[#C5A059] font-bold font-serif ml-4">{item.price}</span></div>
                <p className={`${theme.subText} text-xs leading-relaxed line-clamp-2`}>{item.desc}</p>
              </div>
              <button onClick={() => addToCart(item)} className="mt-4 self-start px-4 py-2 border border-[#C5A059] text-[#C5A059] text-xs font-bold uppercase hover:bg-[#C5A059] hover:text-white transition-colors">Add to Cart</button>
            </div>
          </div>
        ))}
      </div>
...
const SweetsSection = ({ theme, addToCart, sweetsData }) => (
  <div className="pt-32 pb-20">
    <div className="max-w-7xl mx-auto px-6">
      <h4 className="text-[#C5A059] tracking-[0.3em] uppercase text-xs mb-4">Mithai Shop</h4>
      <h2 className={`text-4xl md:text-5xl font-serif mb-12 ${theme.text}`}>Traditional Sweets</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {sweetsData.map((item, idx) => (
          <div key={idx} className="group cursor-pointer">
            <div className="relative overflow-hidden aspect-[4/5] mb-4 rounded-sm bg-gray-800">
              {item.img ? <img src={item.image || item.img} alt={item.name} className="object-cover w-full h-full transform group-hover:scale-110 transition-transform duration-700" /> : <div className="w-full h-full flex items-center justify-center text-gray-500"><Cake /></div>}
              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity" />
              <button onClick={() => addToCart(item)} className="absolute bottom-4 left-4 right-4 bg-white text-black py-3 text-xs tracking-widest uppercase font-bold opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">Add to Cart</button>
            </div>
            <h3 className={`text-lg font-serif mb-1 ${theme.text}`}>{item.name}</h3>
...
const ReviewsSection = ({ theme }) => {
  const reviews = [
    { name: "Rahul Singh", rating: 5, date: "2 days ago", text: "The Paneer Angara is absolutely world-class. Best family restaurant in Jagdishpur!" },
    { name: "Priya Sharma", rating: 5, date: "1 week ago", text: "Celebrated my daughter's birthday here. The decoration and cake were perfect." },
    { name: "Amit Verma", rating: 4, date: "3 weeks ago", text: "Great ambiance and very polite staff. The Soya Chaap is a must-try." },
    { name: "Sneha Gupta", rating: 5, date: "1 month ago", text: "Authentic taste and royal vibes. Love the Kaju Katli from their sweet shop." }
  ];
  return (
    <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto min-h-screen">
      <div className="text-center mb-16"><h4 className="text-[#C5A059] tracking-[0.3em] uppercase text-xs mb-4">Guest Book</h4><h2 className={`text-4xl md:text-5xl font-serif mb-8 ${theme.text}`}>What Our Guests Say</h2></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {reviews.map((review, idx) => (
          <div key={idx} className={`${theme.cardBg} p-8 border ${theme.border} rounded-lg shadow-sm`}>
            <div className="flex justify-between items-start mb-4">
              <div className="flex gap-1 text-[#C5A059]">{[...Array(5)].map((_, i) => (<Star key={i} size={16} fill={i < review.rating ? "currentColor" : "none"} className={i < review.rating ? "text-[#C5A059]" : "text-gray-600"} />))}</div>
              <span className={`text-xs ${theme.subText}`}>{review.date}</span>
            </div>
            <p className={`${theme.text} italic mb-6 leading-relaxed`}>"{review.text}"</p>
            <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-[#C5A059]/20 flex items-center justify-center text-[#C5A059] font-bold font-serif">{review.name[0]}</div><span className={`font-medium ${theme.text}`}>{review.name}</span></div>
          </div>
        ))}
      </div>
    </div>
...
const EventsSection = ({ theme }) => (
  <div className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
      <div>
        <h4 className="text-[#C5A059] tracking-[0.3em] uppercase text-xs mb-4">Celebrations</h4>
        <h2 className={`text-4xl md:text-6xl font-serif mb-8 ${theme.text}`}>Book Your Event</h2>
        <p className={`${theme.subText} mb-8 leading-relaxed`}>From birthday parties with our special cakes to large family dinners, Rajwada provides the perfect royal setting in Jagdishpur.</p>
        <div className="space-y-6">
          <div className={`flex items-center gap-4 ${theme.text}`}><div className="w-10 h-10 rounded-full bg-[#C5A059]/20 flex items-center justify-center text-[#C5A059]"><Cake size={20} /></div><span className="font-medium">Birthday Packages Available</span></div>
          <div className={`flex items-center gap-4 ${theme.text}`}><div className="w-10 h-10 rounded-full bg-[#C5A059]/20 flex items-center justify-center text-[#C5A059]"><Calendar size={20} /></div><span className="font-medium">Priority Table Reservations</span></div>
        </div>
      </div>
      <div className={`${theme.cardBg} p-8 md:p-12 shadow-2xl border ${theme.border} rounded-sm`}>
        <h3 className={`text-2xl font-serif mb-8 text-center ${theme.text}`}>Reservation Form</h3>
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); alert("Thank you! A manager will call you shortly."); }}>
          <input type="text" className={`w-full bg-transparent border-b ${theme.border} py-3 focus:border-[#C5A059] outline-none text-sm ${theme.text}`} placeholder="Your Name" />
          <input type="tel" className={`w-full bg-transparent border-b ${theme.border} py-3 focus:border-[#C5A059] outline-none text-sm ${theme.text}`} placeholder="Phone Number" />
          <div className="grid grid-cols-2 gap-6"><input type="date" className={`w-full bg-transparent border-b ${theme.border} py-3 focus:border-[#C5A059] outline-none text-sm ${theme.subText}`} /><select className={`w-full bg-transparent border-b ${theme.border} py-3 focus:border-[#C5A059] outline-none text-sm ${theme.subText}`}><option>Dinner Table</option><option>Birthday Party</option><option>Bulk Sweet Order</option></select></div>
          <button className="w-full py-4 bg-[#C5A059] text-white tracking-[0.3em] uppercase text-xs font-bold hover:bg-white hover:text-black transition-all">Send Request</button>
        </form>
      </div>
    </div>
  </div>
